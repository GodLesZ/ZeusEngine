...
namespace Zeus.CommunicationFramework.Services.Client {
    ...
    internal class ServiceClient<T> : IServiceClient<T> where T : class {

        /// <summary>
        ///     Underlying client object to communicate with server.
        /// </summary>
        ...
        public ServiceClient(IClient client, object clientObject) {
            _client = client;
        ...
        private void RequestReplyMessenger_MessageReceived(object sender, MessageEventArgs e) {
            ...
            if (_clientObject == null) {
                SendInvokeResponse(invokeMessage, null, new RemoteException("Client does not wait for method invocations by server."));
                return;
            }
            ...
            try {
                ...
                SendInvokeResponse(invokeMessage, null, new RemoteException(innerEx.Message, innerEx));
                return;
            } catch (Exception ex) {
                SendInvokeResponse(invokeMessage, null, new RemoteException(ex.Message, ex));
                return;
            }
